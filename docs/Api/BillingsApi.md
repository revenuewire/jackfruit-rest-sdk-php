...
[**billingInfoUpdatePaypal**](BillingsApi.md#billingInfoUpdatePaypal) | **PUT** /billings/{billingInfoId}/update-paypal | 
...
> \Swagger\Client\Model\BillInfo billingInfoUpdatePaypal($billingInfoId, $ppToken, $token)
...
try {
    $result = $api_instance->billingInfoUpdatePaypal($billingInfoId, $ppToken, $token);
    print_r($result);
} catch (Exception $e) {
